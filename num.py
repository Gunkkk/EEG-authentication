...
def eegtodata5s64hz(a,CLIP_NUM,trainname,shuffle=False):
    ...
    #完成处理 data格式为 （clips*persons=23*subsamples=6*2）*10*64*6*6  =-》828*10*128*6*6
...
def eegtodataseqhznum(a,CLIP_NUM,trainname,len=10,hz=128,shuffle=False):
    num = int(7680/(len*hz))
    b = dict()
    ...
    print(b['data'][0,0,12])
    ##验证
    eeg['data'].resize(CLIP_NUM*23*num,len,hz,6,6)
    print(eeg['data'][0,0,0,1,5],eeg['data'].shape)
    #eeg['data'] = eeg['data'].transpose(0,1,3,4,2)
    #print(eeg['data'][0,0,1,5,0],eeg['data'].shape)
    ...
    #完成处理 data格式为 （clips*persons=23*subsamples=6*2）*10*64*6*6  =-》828*10*128*6*6
    eeg_label = np.zeros(shape=(CLIP_NUM*23,num))
    print(eeg_label.shape)
    print(eeg['label'].size)
    for i in range(eeg['label'].size):
        eeg_label[i,:] = eeg['label'][i]

    eeg_label.resize(CLIP_NUM*23*num,1)
    print(eeg_label.shape)
    #print(eeg_label)
    eeg['label'] = eeg_label
    #       labelge格式为 clips*persons*subsamples*1=-》828*1
